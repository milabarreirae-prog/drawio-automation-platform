...
| **B-05 · Sink Obsidian** | Exportar diagramas como embeds `![[diagrama.drawio]]` + nota `.md` con frontmatter, para la bibliotecaria (knowledge-base-personal-obsidian) | Sinergia de la orquesta; esperar/coordinar el sink de knowledge-base | M | ✅ **RECORRIDO EN FRÍO CERRADO (G-22, 2026-07-29, commit `65e5094`)** — `tests/test_sink_obsidian_recorrido_frio.py` (5 dientes) maneja el CLI vivo (`python -m c4norm` subproceso fresco, nada mockeado) y MIDE el artefacto: DADO un diagrama C4 emitido, el `.md` generado trae frontmatter parseado con `yaml.safe_load`→dict (YAML válido de verdad, no substring) y un embed `![[archivo.drawio]]` extraído por regex propia y **resuelto en disco** (`.exists`/`.is_file` + fidelidad de bytes) en la misma corrida; camino rojo (gemelo_de_error): drawio truncado/vacío → exit≠0, stderr audible, CERO `.md`. Suite 287 passed, ruff limpio. **Pendiente FUERA de este viaje**: el *contrato final* del vault (nombres de frontmatter, taxonomía, entrega) lo define la bibliotecaria — hoy el frontmatter nace `contrato_vault: por_validar`; la escritura real dentro del vault queda FUERA (ver sección siguiente). |
