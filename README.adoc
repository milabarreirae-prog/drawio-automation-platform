...
This repository now also carries a Python-based automation platform on top of the
upstream headless Draw.io image.

The added platform provides:

* FastAPI REST API for diagram submission and status lookup
* Async worker processing with Redis/ARQ
* Compliance validation for XML, colors, stencils, and ArchiMate licensing
* Stencil resolution helpers, webhook callbacks, and object-storage integration

Fork-specific documentation:

* link:ARCHITECTURE.md[ARCHITECTURE.md]
* link:NOTICE[NOTICE]
* link:THIRD_PARTY_LICENSES.md[THIRD_PARTY_LICENSES.md]
* link:FORK_SETUP.md[FORK_SETUP.md]

Operational branch model:

* `main` is the primary integration and release branch for this fork.
* `upstream/v1.x` is kept as the synchronization source for the upstream runtime lineage.
